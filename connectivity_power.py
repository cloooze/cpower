...
def main():
	...
	if (order_id or source_api) == None:
		logging.error('orderId or sourceApi not found in environment variables. Stopping script execution.')
		exit('FAILURE')
	...
	try:
		...
		logging.exception('Impossible to contact ECM. Stopping script execution.')
		exit('FAILURE')
			...
			if operation == 'create':
				...
				if step == None:
					try:
						...
						ecmutil.check_ecm_order_status(order_resp)
					...
					if cc.rowcount > 0:
						...
						try:
							...
							logging.error('Could not insert the following data to cpower table %s. Stopping script execution.' % (entry,))
							exit('FAILURE')
