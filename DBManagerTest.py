...
class DBManagerTest(unittest.TestCase):
	...
	def test_query_customer_02(self):
		tuple = ('test2', 'test2')
		self.assertIsNotNone(self.dbman.query('''INSERT INTO customer VALUES(?, ?)''', tuple))
		c_id = ('test2', )
	...
	def test_query_network_service_01(self):
		tuple = ('test2', 'test2', 'test2', 'test2', 'test2', 'test2')
		self.assertIsNotNone(self.dbman.query('''INSERT INTO network_service VALUES(?, ?, ?, ?, ?, ?)''', tuple))
		c_id = ('test2', )
	...
	def test_query_vnf_01(self):
		tuple = ('test2', 'test2', 'test2', 'test2', 'test2')
		self.assertIsNotNone(self.dbman.query('''INSERT INTO vnf VALUES(?, ?, ?, ?, ?)''', tuple))
		c_id = ('test2', )
	...
	def test_query_vn_group_01(self):
		tuple = ('test2', 'test2', 'test2', 'test2', 'test2')
		self.assertIsNotNone(self.dbman.query('''INSERT INTO vn_group("VNF_ID", "VN_LEFT_ID", "VN_LEFT_NAME", "VN_RIGHT_ID", "VN_RIGHT_NAME") VALUES(?, ?, ?, ?, ?)''', tuple))
	...
	def test_save_network_service_01(self):
		tuple = ('ntw_002', 'ntw_002','ntw_002','ntw_002','ntw_002','ntw_002',)
		self.dbman.save_network_service(tuple)
		c_id = ('ntw_002', )
		self.dbman.query('''SELECT * FROM network_service WHERE ntw_service_id=?''', c_id)
		self.assertEqual('ntw_002', self.dbman.fetchone()['ntw_service_id'])
		
	def test_save_vnf_01(self):
		tuple = ('vnf_002', 'vnf_002','vnf_002','vnf_002','vnf_002')
		self.dbman.save_vnf(tuple)
		c_id = ('vnf_002', )
		self.dbman.query('''SELECT * FROM vnf WHERE vnf_id=?''', c_id)
		self.assertEqual('vnf_002', self.dbman.fetchone()['vnf_id'])

	def test_save_vn_group_01(self):
		tuple = ('vn_001', 'vn_001', 'vn_001', 'vn_001', 'vn_001')
		self.dbman.save_vn_group(tuple)
		self.dbman.query('''SELECT * FROM vn_group WHERE vn_group_id=1''')
		self.assertEqual(1, self.dbman.fetchone()['vn_group_id'])
	...
	def test_fetchall_01(self):
		tuple_1 = ('customer_001', 'customer_001_name')
		tuple_2 = ('customer_002', 'customer_002_name')
		self.dbman.save_customer(tuple_1)
		self.dbman.save_customer(tuple_2)
